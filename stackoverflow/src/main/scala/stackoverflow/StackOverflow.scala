...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.map{ case (_, pairs) => (pairs.head._1, answerHighScore(pairs.map(_._2))) }
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored.flatMap { case (p, s) =>
        firstLangInTag(p.tags, langs) match {
          case Some(index) => Some((index * langSpread, s))
          case _ => None
        }
      }
      .persist()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    def calcMedian(s: Seq[Int]): Int = {
      val (lower, upper) = s.splitAt(s.size / 2)
      if (s.size % 2 == 0) (lower.last + upper.head) / 2 else upper.head
    }

    val median = closestGrouped.mapValues { vs =>
      val langGroups: Map[Int, Iterable[(Int, Int)]] = vs.groupBy(_._1)
      val maxLang: (Int, Iterable[(Int, Int)]) = langGroups.maxBy(_._2.size)

      val langLabel: String   = langs(maxLang._1 / langSpread) // most common language in the cluster
      val langPercent: Double = maxLang._2.size.toDouble * 100 / vs.size // percent of the questions in the most common language
      val clusterSize: Int    = vs.size
      val medianScore: Int    = calcMedian(vs.map(_._2).toSeq.sorted)
