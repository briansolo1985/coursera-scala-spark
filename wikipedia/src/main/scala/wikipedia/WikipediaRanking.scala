...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[6]").setAppName("wikipedia-ranking")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((n, wa) => n + isPresent(lang, wa.text), _ + _)

  private def isPresent(lang: String, text: String) = if(text.split(" ").contains(lang)) 1 else 0
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(wa => langs.flatMap(lang => if(isPresent(lang, wa.text) == 0) None else Some(lang, wa)))
      .groupByKey()
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.map{ case (k,it) => (k, it.size) }.collect.toList.sortBy(-_._2)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(wa => langs.map(lang => (lang, if(isPresent(lang, wa.text) == 0) Nil else Iterable(wa))))
      .reduceByKey(_ ++ _)
      .map{ case (k,it) => (k, it.size) }
